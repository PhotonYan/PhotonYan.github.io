...
我们称这种用*一个覆盖原集合的开区间族*的长度衡量原集合长度的集合长度测量方式为**外测度**，上述开区间族被称作原集合的一个**开覆盖**。「外测度」的外字来源于我们是「从外面包裹住集合来测量」的，这与Riemann积分中我们从集合内部进行分划的「内测度」相对。
...
2. 代数对补集具有封闭性：$\forall P\in\mathcal A,\, X- P\in A$;
3. 代数对并集封闭（有限可加性）：$P\in\mathcal A,\, Q\in\mathcal A\Rightarrow P\cup Q\in \mathcal A$.

如果第三个条件被增强为**可数可加性**

测度实际上就是我们上述积分过程中出现的函数$m$。$m$是一个将集合映射为实数（长度）的函数。Riemann积分中，我们已经将开区间长度$(x_i,x_{i+1})$定义为$|x_{i+1}-x_i|$，我们希望测度函数能够在此基础上进行推广。因此我们要求，如果一个函数$\mu(A)$是测度函数，那么它必须要满足以下性质：
1. 考虑集合$X$与其上的$\sigma$-代数$S$，测度$\mu$是$S\rightarrow[0,\infty]$的函数；
2. 
